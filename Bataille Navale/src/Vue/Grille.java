	...
	public void checkAliveBateau() {
		...
		for (int i = 1; i < 16; i++) {
			for (int j = 1; j < 16; j++) {
				...
				if (buttons1[i][j].getText().contains("s") && buttons1[i][j].getForeground() != Color.RED) {
					sIA.setIsAlive(true);
				}
				if (buttons1[i][j].getText().contains("b") && buttons1[i][j].getForeground() != Color.RED) {
					cIA.setIsAlive(true);
				}
				if (buttons1[i][j].getText().contains("d") && buttons1[i][j].getForeground() != Color.RED) {
					dIA.setIsAlive(true);
				}
				if (buttons1[i][j].getText().contains("c") && buttons1[i][j].getForeground() != Color.RED) {
		...
		while (!possiblemove) {
			...
			if (down) {
				...
				response = JOptionPane.showOptionDialog(null,
						"Votre " + aBateau.getNom() + " est en mouvement ! Où voulez-vous le déplacer ?",
						"Amiral, Déplacez votre navire !", JOptionPane.DEFAULT_OPTION, JOptionPane.PLAIN_MESSAGE, null,
						options, options[0]);
				...
				if (response == 0) {
					...
					if (test1 && test2) {
						...
						JOptionPane.showMessageDialog(null,
								"Aucun déplacement possible, veuillez tirer avec ce bateau");
					}
	...
	public void Tir() {
		...
		while (this.isCaseIsadv() == false) {
			...
			if (getCaseclicked() != null) {
				...
				} else if (Integer.valueOf(grillenumber) == 1 && getButtons()[grillex][grilley].getText().contains("c")
						&& !moved && getCurrentboat().getNom() == "Cruiser") {
					checkPossibleMove(currentboat, "c");
					moved = true;
				} else if (Integer.valueOf(grillenumber) == 1 && getButtons()[grillex][grilley].getText().contains("d")
						&& !moved && getCurrentboat().getNom() == "Destroyer") {
					checkPossibleMove(getCurrentboat(), "d");
					moved = true;
				} else if (Integer.valueOf(grillenumber) == 1 && getButtons()[grillex][grilley].getText().contains("b")
						&& !moved && getCurrentboat().getNom() == "Battleship") {
					checkPossibleMove(getCurrentboat(), "b");
					moved = true;
				} else if (Integer.valueOf(grillenumber) == 1 && getButtons()[grillex][grilley].getText().contains("s")
						&& !moved && getCurrentboat().getNom() == "Submarine") {
			...
			for (int y = grilley - getCurrentboat().getImpactrange(); y <= grilley
				...
				if (i > 0 && i < 16 && y > 0 && y < 16) {
					if (getButtons1()[i][y].getText().contains("-")
							&& getButtons1()[i][y].getForeground() != Color.red) {
